...
def run_pipeline(row, out_dir, base_data_dir, batch):
    ...
    if row.QC == "N": return None

    # load data using same function
    d = load_data(f'{base_data_dir}/{center_to_process}/{subID}', subID, center_to_process)
    ...
    # WE ASSUME THAT FASTSURFER HAS BEEN RUN FOR ALL SUBJECTS
    # if it doesnt find it, continue
    ...
    # Run the batch
    # parameters
    matlab_path = "'/home/extop/lib/MATLAB/R2021b/bin/matlab'"
    conn_path = "'/home/extop/GERARD/CONN'"
    ...
    if not os.path.exists(f'{out_dir_subject}/conn_FC/results/preprocessing/ROI_Subject001_Condition000.mat'): 

        function_call = "Base_batch('" + func_path + "', '" + recon_all_path + "', '" + out_dir_subject + "', str2num('" + str(TR) +"'), " + str(sliceorder) + ", " + str(scans_to_remove) + ");"
        # run
        print(f'export MATLABPATH={conn_path}; {matlab_path} -nosplash -nodisplay -nodesktop -r "cd {out_dir_subject}; {function_call}"')
        #redirect all output to file
        # with open(output_file, 'w') as f:
        cmd = subprocess.Popen(f'export MATLABPATH={conn_path}; {matlab_path} -nosplash -nodesktop -r "cd {out_dir_subject}; {function_call}exit;"', 
...
# python run_CONN.py --in_dir /mnt/nascarm01/data/Projectes/MAGNIMS2021 --in_csv /home/extop/GERARD/DATA/MAGNIMS2021/data_total.csv --out_dir /home/extop/GERARD/DATA/MAGNIMS2021 --center NAPLES --batch nyi --njobs=1
...
parser.add_argument('--center', type=str, required=True, help="Select the centers to use")
parser.add_argument('--batch', type=str, required=True, help="location of the .mat file with the base batch information")
parser.add_argument('--njobs', type=int, required=True, default=1, help="Number of jobs to use")
...
outputs = Parallel(n_jobs=args.njobs, backend="threading")(delayed(run_pipeline)(row, out_dir, base_data_dir, batch) for row in df_connect_todo.itertuples())
