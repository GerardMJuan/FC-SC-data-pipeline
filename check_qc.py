...
def check_FastSurfer(subj_dir, out_dir, subID):
    ...
    if not (os.path.exists(t1_path) and os.path.exists(brain_path) and os.path.exists(seg_path)):
        raise FileNotFoundError("Files generated by FS not found. Make sure that the pipeline has run correctly!")
    ...
    mrivis.color_mix(brain, seg, num_slices=10, alpha_channels=(1, 1), output_path=f'{out_dir}/{subID}_1fs_seg_overlay2')
    plt.close()
...
def check_LesionSeg(subj_dir, out_dir, subID):
    ...
    t1_path = f'{recon_all_path}/mri/T1.nii.gz'
    flair_path = None
    if len(glob.glob(f'{subj_dir}/lst/FLAIR2T1.nii.gz')) > 0: flair_path = f'{subj_dir}/lst/FLAIR2T1.nii.gz'
    elif len(glob.glob(f'{subj_dir}/anat/FLAIR2T1.nii.gz')) > 0: flair_path = f'{subj_dir}/anat/FLAIR2T1.nii.gz'
    ...
    if flair_path is not None:
        ...
        # plt.close()
        mrivis.color_mix(flair, seg_dil, num_slices=10, alpha_channels=(1, 0.3), output_path=f'{out_dir}/{subID}_2flair_lesionseg_dil')
        plt.close()
    else:
        # if flair not available
        # mrivis.color_mix(t1, seg, num_slices=10, alpha_channels=(1, 0.2), output_path=f'{out_dir}/2flair_lesionseg')
        # plt.close()
        mrivis.color_mix(t1, seg_dil, num_slices=10, alpha_channels=(1, 0.3), output_path=f'{out_dir}/{subID}_2flair_lesionseg_dil')
        plt.close()

    if flair_path is not None:
        mrivis.checkerboard(flair, t1, patch_size=15, output_path=f'{out_dir}/{subID}_2flair_t1_check')
        plt.close()
...
def check_DTMask(subj_dir, out_dir, subID):
    ...
    mrivis.color_mix(dwi_mask, dwi, num_slices=10, alpha_channels=(0.3,1.0), output_path=f'{out_dir}/{subID}_3dtmask_checkbiascorrect')
    plt.close()
...
def check_DTrecon(subj_dir, out_dir, subID):
    ...
    # check paths existing
    if not (os.path.exists(og_b0) and os.path.exists(proc_b0) and os.path.exists(FA) and os.path.exists(seg5tt) and os.path.exists(seg5tt_wm) and os.path.exists(seg5tt_csf) and os.path.exists(t1)):
        raise FileNotFoundError("Files generated by DTRecon not found. Make sure that the pipeline has run correctly!")
    ...
    # TODO DO COLOR MIX
    mrivis.color_mix(t1, seg5tt_wm, num_slices=10, alpha_channels=(1,1), output_path=f'{out_dir}/{subID}_dt_reg_t1_wm')
    ...
    # registration between T1 and wm (color mix)
    # TODO DO COLOR MIX
    mrivis.color_mix(t1, seg5tt_csf, num_slices=10, alpha_channels=(1,1), output_path=f'{out_dir}/{subID}_dt_reg_t1_csf')
    ...
    mrivis.color_mix(proc_b0, seg5tt, num_slices=10, alpha_channels=(1,0.7), output_path=f'{out_dir}/{subID}_2dt_t1_seg_overlay')
    plt.close()
...
def check_Tracking(subj_dir, out_dir, subID):
    ...
    # check paths existing
    if not (os.path.exists(dwi_proc) and os.path.exists(tracks_AEC) and os.path.exists(sift_weights)):
        raise FileNotFoundError("Files generated by DTRecon not found. Make sure that the pipeline has run correctly!")

    output_file = f'{out_dir}/log_qctrack.txt'
    #redirect all output to file
    with open(output_file, 'w') as f:
        cmd  = subprocess.Popen(f'exec scripts/QC_track.sh {subj_dir} {out_dir} {subID}', shell=True, stdout=f, stderr=f)
        cmd.wait()
    ...
    mrivis.checkerboard(dwi_proc, anat2diff, patch_size=15, output_path=f'{out_dir}/{subID}_3track_anat2diff_checkerboard')
    plt.close()

    mrivis.color_mix(anat2diff, dwi_proc, num_slices=10, alpha_channels=(1,0.7), output_path=f'{out_dir}/{subID}_4track_t1_seg_overlay')
    plt.close()

    mrivis.color_mix(dwi_proc, nodes2diff, num_slices=10, alpha_channels=(1,0.7), output_path=f'{out_dir}/{subID}_track_nodes2diff_overlay')
...
def check_fMRI(subj_dir, out_dir, subID):
    ...
    if not (os.path.exists(fmri_mean_brain) and os.path.exists(t1)):
        raise FileNotFoundError("Files generated by fMRIPROC not found. Make sure that the pipeline has run correctly!")
...
def plot_connectivity(subj_dir, out_dir, subID):
    ...
    if not (os.path.exists(SC_path_weight) and os.path.exists(SC_path_length)):
        raise FileNotFoundError("SC matrices not found. Make sure that the pipeline has been run correctly!")
    ...
    plt.subplot(121), plt.title('weights'), plt.imshow(SC_weight, interpolation='none'), plt.colorbar()
    plt.subplot(122), plt.title('lengths'), plt.imshow(SC_length, interpolation='none'), plt.colorbar()
    ...
    plt.subplot(121), plt.title('Corr'), plt.imshow(r_mat, interpolation='none'), plt.colorbar()
    plt.subplot(122), plt.title('Norm Corr'), plt.imshow(zr_mat, interpolation='none'), plt.colorbar()
...
@click.command(help="Perform a quality check of the preprocessing pipeline over all subjects. By default, .")
@click.option("--out_dir", default="qc")
@click.option("--subj_list", type=click.STRING, help="Txt list with the subjects we want to qc")
...
def run_all_subjects(subj_dir, out_dir, subj_list, pip_csv):
    ...
    if subj_list is not None and os.path.exists(subj_list):
        ...
        for s in subj_list:
            ...
            df_pipeline_todo = df_pipeline_todo.append(df_pipeline[(df_pipeline.id == s[0]) & (df_pipeline.CENTER == s[1])]) # will select subjects taht we don't have to process
