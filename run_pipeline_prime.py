...
# example call
# python run_pipeline_prime.py --in_dir /mnt/nascarm01/data/Projectes/MAGNIMS2021 --in_csv /home/extop/GERARD/DATA/MAGNIMS2021/data_total.csv --in_pip /home/extop/GERARD/DATA/MAGNIMS2021/pipeline.csv --out_dir /home/extop/GERARD/DATA/MAGNIMS2021 --njobs=1 --subj_list test_subj.txt -fmri -tvb


def run_pipeline(row, df_pip_status, out_dir, subj_list, base_data_dir, fs, lst, dt, tck, tvb):
    """
    ...
    # (temporal) directory where we will work
    # hardcoded because yes
    working_dir_raw="/home/extop/GERARD/DATA/MAGNIMS2021/raw"


    ### sub-MS0186 DOESNT WORK
    if subID == "FIS_083": return 0
    if subID == "FIS_121": return 0
    if subID == "sub-MS0186": return 0
    if subID == "sub-0010" and type_dir == "MAINZ": return 0
    if subID == "sub-0026" and type_dir == "MAINZ": return 0
    if subID == "sub-0010" and type_dir == "MAINZ": return 0


    # information about pipeline steps
    fastsurfer_status = df_pip_status[(df_pip_status.id==subID) & (df_pip_status.CENTER==type_dir)]["fastsurfer"].bool()
    dt_status = df_pip_status[(df_pip_status.id==subID) & (df_pip_status.CENTER==type_dir)]["DWI_preproc"].bool()
    sc_status = df_pip_status[(df_pip_status.id==subID) & (df_pip_status.CENTER==type_dir)]["agg_SC"].bool()
    tvb_status = df_pip_status[(df_pip_status.id==subID) & (df_pip_status.CENTER==type_dir)]["toTVB"].bool()
    ...
    if not (do_fs or do_dt or do_tck or do_tvb or do_lst): return 0
    ...
    if do_fs:
        ...
        with open(output_file, 'w') as f:
            cmd = subprocess.Popen(f'exec scripts/FastSurfer.sh {subID} {d["T1w"]} {out_dir_subject}', shell=True, stdout=f, stderr=f)
            cmd.wait()
    ...
    if do_lst and os.path.isfile(d["FLAIR"]):
        ...
        with open(output_file, 'w') as f:
            cmd = subprocess.Popen(f'exec scripts/runLST.sh {subID} {d["T1w"]} {d["FLAIR"]} {out_dir_subject} {type_dir}', shell=True, stdout=f, stderr=f)
            cmd.wait()
    ...
    if do_dt:   
        ...
        with open(output_file, 'w') as f:
            cmd  = subprocess.Popen(f'exec scripts/DT_recon.sh {subID} {out_dir_subject} {d["DWI"]} {d["DWI2"]} {d["bval"]} {d["bvec"]} {d["Lesions"]}\
    ...
    if do_tck:
        ...
        #redirect all output to file
        with open(output_file, 'w') as f:
            cmd = subprocess.Popen(f'exec scripts/Tracking.sh {subID} {out_dir_subject}', 
                                shell=True, stdout=f, stderr=f)
            cmd.wait()
...
parser.add_argument('--in_dir', type=str, required=True, help="input dir with subject data, general input dir (MAGNIMS2021")
parser.add_argument('--in_csv', type=str, required=True, help="csv with the subject info (general csv)")
parser.add_argument('--in_pip', type=str, required=True, help="csv with the information of the current state of the pipeline")
parser.add_argument('--out_dir', type=str, required=True, help="A string argument (also general directory)")
parser.add_argument('--njobs', type=int, default=1, help="Number of jobs to use")
parser.add_argument('--subj_list', type=str, help="A text file with a list of subjects, one per line")
...
if args.subj_list:
    ...
    for s in subj_list:
        ...
        df_connect_todo = df_connect_todo.append(df_connect[(df_connect.SubjID == s[0]) & (df_connect.CENTER == s[1])]) # will select subjects taht we don't have to process
else:
    subj_list = []
...
############### 
outputs = Parallel(n_jobs=args.njobs, backend="threading")(delayed(run_pipeline)(row, df_pip_status, out_dir, subj_list, base_data_dir, fs, lst, dt, tck, tvb) for row in df_connect_todo.itertuples())